...
`themes.html` and `img/worlds/` are regenerated together by
`scripts/public-world-gallery.sh`. The command reads the ordered roster from
the product's `--list-worlds` door and captures one authored Markdown specimen
through the real headless renderer. Do not edit the page or captures by hand.
