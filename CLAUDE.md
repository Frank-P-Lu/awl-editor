...
- Test-global locking: one process-wide reentrant guard, `crate::testlock::serial()`, taken by every test and every `cfg(test)` global writer. Three rounds of ABBA deadlocks retired the ordered per-module locks; don't reintroduce them. (`config::ENV_LOCK` separately serializes env mutation.) Its outermost guard also RESTORES the active world on drop (`theme::WorldPin`) — serializing the world was never enough: ~20 tests swapped worlds and never swapped back, which made a render law pass or fail on test ORDER. `theme_global_law` sweeps `src/` so a test-side world swap outside the guard can't return. TWO DOORS: tests take `serial()` (pins the world); `cfg(test)` acquires that live in PRODUCTION code (`apply_core`, the `page`/`menubar` writers) take `serial_nopin()` — same lock, no restore, because a world the ACTION set (the theme picker's live preview) must survive the action. The textual sweep can't see that one (`apply_core` contains an acquire); `theme_global_law::a_world_a_production_action_sets_survives_that_action` is the runtime companion.
