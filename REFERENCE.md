...
| Move file to Trash | Move a clean file to the operating system's recoverable Trash. Save or resolve any changes first, then invoke this command again. | — | — | Native |
...
| `--seed-tree` | `DIR` | seed a whole fixture PROJECT (DIR's files, recursively, at their own paths) into a hermetic scenario sandbox — the only way a --screenshot-app run can open a SECOND file, since --root alone seeds an empty directory marker; refused outside a hermetic door |
