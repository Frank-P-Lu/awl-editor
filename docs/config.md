...
- **Keymap flavor (`keymap = "native" | "emacs"`):** a whole-catalog preset over the `linux_keep_emacs` machinery. `"emacs"` widens the effective keep-list to every displaced letter ∪ the user's own entries, EXCEPT `C-c`/`C-v` (the native-clipboard carve-out below) — `Config::effective_linux_keep()` is the one owner of the composition, filtering those two out of the preset's own contribution before the union; every call site reads the composed result, never `config.linux_keep_emacs` directly. On Linux, `"emacs"` also seeds a classic Meta (Alt) layer (`keymap::platform::LINUX_EMACS_META_SEED`, `KeymapState::seed_defaults`'s `linux_emacs_meta` gate): `M-x` command palette, `M-w` copy, `M-f`/`M-b` word motion, `M-d`/`M-Backspace` word delete, `M-v` page up, `M-<`/`M->` document ends — each firing an existing catalog `Action`, never a parallel dispatch path. Also a Settings "Keymap" row — a `SettingKind::Picker` (a "Keymap…" catalog command, `COVERED_BY`-suppressed from the plain palette row, descends into a native/emacs sub-overlay; accept applies, persists, and names the resulting layout with a notice — no silent flip). Hidden entirely on `Convention::Mac`, where the flavor (keep-list AND Meta layer alike) is structurally inert — Option keeps typing accented characters there, and native ⌘ bindings double-fire the emacs slot regardless.
- **`linux_keep_emacs` (per-chord door):** on Linux, native-wins displaces the bare-control emacs cluster (`C-f`/`C-b`/`C-n`/`C-p`/`C-a`/`C-e`). This array lists chords that keep their emacs meaning under `Convention::Linux` only. Mac is inert (gated on `convention == Linux`). `C-c`/`C-v` stay native under EITHER flavor by construction (Omarchy/Hyprland forwards Super+C/V as Ctrl+C/V for the system clipboard) — naming them here explicitly is still honoured, since this per-chord door is unfiltered, unlike the `emacs` flavor preset's own contribution. `C-x` stays the emacs prefix under `keymap = "emacs"` (it carries Save/Open); `[keys] cut = "C-x"` trades the prefix back for native Cut.
