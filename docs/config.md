...
- **Keymap flavor (`keymap = "native" | "emacs"`):** a whole-catalog preset over the `linux_keep_emacs` machinery. `"emacs"` widens the effective keep-list to every displaced letter ∪ the user's own entries. `Config::effective_linux_keep()` is the one owner of the composition — every call site reads it, never `config.linux_keep_emacs` directly. Also a Settings "Keymap" row — a `SettingKind::Picker` (a "Keymap…" catalog command, `COVERED_BY`-suppressed from the plain palette row, descends into a native/emacs sub-overlay; accept applies, persists, and names the resulting layout with a notice — no silent flip). Hidden entirely on `Convention::Mac`, where the flavor is structurally inert (native ⌘ bindings double-fire the emacs slot regardless).
