...
|---|---|---|
| Cut a public tag at all | **settled — tags are cut.** `v0.9.0`, `v0.10.0` and `v0.11.0` are published, each Linux-only and marked prerelease. Every tag still waits on the user's explicit word, every time | the user, explicitly (CLAUDE.md §Branches) |
