...
| **Warped grid** (item 132) | ONE curved tunnel in perspective, seen through the two Frame margins as two composed slices of the same field. There is no geometry and no 3-D engine: cross-section "rings" are level sets of `log(radius)` around a single vanishing point, so their projected spacing tightens with depth exactly as perspective requires, and longitudinal "rails" are level sets of the polar angle. Forward travel is one subtraction from the ring coordinate. The useful curvature is distorted OUTWARD — a fixed share of the page's own half-width is removed from every horizontal distance — because composed literally the vanishing region would sit behind the opaque page and every travel cue would be lost. One shared vanishing point is what makes the two margins one journey: a left bend moves it left, so the left slice's radii shrink and its lines tighten while the right slice's grow and open, with no per-side rule. Every fifth line is stronger; the minor rung retires at narrow margins and both lattices retire into the vanishing region, so the field can never pack past resolvable. Camera travel comes from the shared ambient tick and a deterministic 7-minute route. | Kite |
