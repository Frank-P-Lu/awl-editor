...
| **Warped grid** (items 132/194) | ONE camera, ONE projected cylinder, cropped at the page. There is no geometry and no 3-D engine: the camera sits inside a tube whose axis is a parabolic arc, so a wall point at depth `z` projects to `p = B/u + u*e(theta)` with `u` the cross-section's projected radius. That makes a cross-ring an exact ellipse of radius `u` centred at `B/u` and a longitudinal rail the curve both of them migrate along — so a bend moves and warps the WHOLE opening off-centre as a unit, the near wall broadening continuously around the outside of the turn while the far wall compresses continuously around the inside. Nothing pinches or steers per margin: the opaque page simply crops the one field, and the vanishing region stays honestly behind it. The section is sized against the page rather than authored — three page widths across, so the page hides a third of it and each margin reads two thirds, its own third plus the third behind the page — and flattened by one affine transform until its flank fits the room. Forward travel is one subtraction from the ring coordinate. Every fifth line is stronger; the minor rung retires at narrow margins and both lattices retire into the far end, so the field can never pack past resolvable. Camera travel comes from the shared ambient tick and a deterministic 7-minute route. | Kite |
...
### Kite
**A near-white mineral page gliding through a cool warped-grid tunnel — indigo geometry in the margins, one vermilion eye.**
Fira Sans's screen-engineered humanist sans on pale lavender; Noto Sans JP for Japanese; JetBrains Mono for code.
Kite is the deliberate LIGHT counterpart to Firetail, and it is defined against it on every axis: cool rather than warm, geometric rather than organic, crisp rather than liquid, directional rather than drifting. The writing page stays flat, opaque and motionless; the whole world is in the Frame. Both margins are crops of ONE curved tunnel drawn in indigo and graphite — one camera, one opening, which shifts off-centre as a unit when the route bends — and the camera travels continuously forward, easing into a new long direction — straight, a left or right bend, a climb, a descent — roughly once a minute over a deterministic seven-minute route that closes on itself invisibly. Losing focus pauses it in place and it resumes without catching up; ambient motion off and Reduce Motion freeze it to one composed still. The single accent is a hot vermilion caret — the kite's red eye. Australian kites hang almost motionless on a thermal and then bank away without a wingbeat; the world is that gliding turn read as a room.
Modern.
