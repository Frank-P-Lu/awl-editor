...
Three checks, run against one candidate PNG + its sidecar JSON:
  ...
  3. Legibility arithmetic reported (not gated — taste stays the user's
     ...
     relative-luminance formula) for every non-empty body-text row, sampling
     the same full-resolution ink pixels back off a box-downscaled thumbnail —
     this is what a heavily downscaled link-unfurl actually shows, not the
     sharp full-res ink color.
...
def report(png_path, json_path, label):
    ...
    body_rows = [
        ...
        and r.get("xs")
        and max(r["xs"]) - min(r["xs"]) > 1.0
    ]
    if body_rows:
        measurements = [
            (thumb_contrast(rows, bpp, w, h, row), row) for row in body_rows
        ]
        (ratio, ink, bg, n), row = min(measurements, key=lambda item: item[0][0])
        if n == 0:
            ok = False
            print(
                "FAIL: thumbnail contrast sampled zero ink pixels on the selected "
                f"body row (line {row['line']})."
            )
        wcag = "PASS (>=4.5 WCAG AA)" if ratio >= 4.5 else "below WCAG AA 4.5:1"
        print(
            f"~360px-thumbnail minimum body-text contrast: {ratio:.2f}:1 "
            f"({wcag}; line {row['line']}, {n} ink samples)"
        )
    else:
        ok = False
        print("FAIL: no non-empty body rows were available for contrast measurement.")
    print()
    return ok
