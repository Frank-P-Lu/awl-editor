...
# ---------------------------------------------------------------------------
# THE RECOVERY BAND IS THE MEASURED SINGLE-WORKTREE YIELD, NOT A HABIT.
#
# sweep.sh prunes the calling worktree alone, so the gap between the floor this
# script starts recovering at and the floor it refuses at is worth exactly one
# worktree's sweep. The band outlived the fleet-wide sweep it was sized for
# once already; the derivation is read back off a receipt rather than trusted
# to whoever edits the constants next.
#
# Read from the receipt, not from the source text: the published number is what
# a future tuner will act on, and a constant that never reaches the receipt is
...
printf '%s\n' "$healthy_bytes" >"$WORK/free"
band_receipt="$(run)"
band_healthy="$(band_field healthy_bytes "$band_receipt")"
band_minimum="$(band_field minimum_bytes "$band_receipt")"
band_yield="$(band_field sweep_yield_bytes "$band_receipt")"
# Presence floor first. A zero yield collapses the band to nothing and would
# satisfy the equality below for free, reporting the same green as a measured
# one — the failure shape where a law is satisfiable by deleting its subject.
[[ "$band_yield" =~ ^[0-9]+$ ]] && (( band_yield > 0 )) || {
  echo "test-disk-preflight: the receipt published sweep_yield_bytes=$band_yield; a zero or absent yield makes the band law vacuous" >&2
  exit 1
}
(( band_healthy - band_minimum == band_yield )) || {
  echo "test-disk-preflight: recovery band is $(( band_healthy - band_minimum )) bytes wide but one worktree's sweep is measured at $band_yield — a band wider than the yield locks and traverses for a recovery that cannot arrive" >&2
  exit 1
}

# What the sweep actually gave back, reported on both the success and the
# failure path. The arithmetic is the assertion: a sign error or a stale
# before-reading is invisible in a status word and obvious in a byte count.
printf '%s\n' 1073741824 >"$WORK/free"
: >"$WORK/sweeps"
reclaim_receipt="$(run "$healthy_bytes")"
[[ "$reclaim_receipt" == *"reclaimed_bytes=$(( healthy_bytes - 1073741824 ))"* ]] || {
  echo "test-disk-preflight: recovery did not report what it reclaimed: $reclaim_receipt" >&2
  exit 1
}
printf '%s\n' "$insufficient_bytes" >"$WORK/free"
: >"$WORK/sweeps"
if run "$insufficient_bytes" >"$WORK/barren.out" 2>"$WORK/barren.err"; then
  echo "test-disk-preflight: a barren sweep below the minimum unexpectedly passed" >&2; exit 1
fi
grep -Fq 'reclaimed_bytes=0' "$WORK/barren.err" || {
  echo "test-disk-preflight: a sweep that reclaimed nothing did not say so — this is the common case on an active lane, and the number is what the next tuning pass needs: $(cat "$WORK/barren.err")" >&2
  exit 1
}
