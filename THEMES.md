...
| `list_style` | `Pane` \| `Bars` \| `Diagonal(spine)` \| `Rules(weight)` | How a summoned picker draws the surfaces behind its candidate rows — one pane (default) vs. per-row plates that grow under the selection. | `Bars` — Firetail, Galah; `Diagonal` — Magpie, Mangrove; `Rules` — Paperbark |
