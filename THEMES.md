...
- **A ground** (`Background`): the procedural margin pattern (Dots / Gradient /
  ...
  low-coverage mark over a two-color gradient. **Item 87 (2026-07)** gave `Waves`
  alone a very slow, seamless horizontal PHASE DRIFT on top of that otherwise-static
  shape (palette, band count, and the settled/theme-crossing composition stay
  byte-identical) — see "The waves phase-drift law" below.
