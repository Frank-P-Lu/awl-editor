...
| `placard_placement` | `Contained` \| `Bleed { x_em, y_em }` | Whether a placard remains inside its canvas-corner inset or receives an authored em-relative offset after that anchor. The reported wordmark geometry, raster material and rotated locator all read the displaced rect. | `Bleed` — Cassowary |
| `summoned_material` | `Flat` \| `Scanlines { pitch_px, line_px, strength }` | A static absolute-canvas raster laid over a summoned pane and its placard. Pitch and line weight are display-density lengths; one phase spans every rect, and the material owns no clock or random input. | `Scanlines` — Cassowary |
...
| `list_style` | `Pane` \| `Bars` \| `Diagonal(spine)` \| `Ruled(weight)` | How a summoned picker draws the surfaces behind its candidate rows — one pane (default) vs. per-row plates that grow under the selection. | `Bars` — Firetail, Galah; `Diagonal` — Magpie, Mangrove; `Ruled` — Kite, Paperbark |
| `pane_split` | `Split` \| `Unified` | Whether a card-backed picker separates its query beat from the rows or draws one exterior pane. Bare list styles ignore it. | `Unified` — Cassowary |
| `facet_style` | `Text` \| `Band` \| `DockedTab` \| `Chips(variant)` | How the faceted picker's lens strip skins its labels. `DockedTab` seats the shaped strip against the pane edge and joins the active label to the card border without changing its hit span. | `Band` — Kite; `DockedTab` — Cassowary; `Chips` — Firetail, Galah, Magpie, Mangrove |
| `location_style` | `Inline` \| `RotatedRail(LocationLabelStyle)` \| `Raked(LocationLabelStyle)` | How the active lens's secondary location cue draws. Non-inline treatments author face (`Body` / `Mono` / `Chrome`), expression-relative scale, palette-role ink, real shaper tracking in em, and locator grammar. `IndexOnly` and `Indexed` derive the truthful one-based position from the active lens strip and park without one. Cassowary authors mono/0.28/muted/0.06em/`NN`; Magpie authors its prior chrome/0.92/muted→ink/untracked label treatment. | `RotatedRail` — Cassowary; `Raked` — Magpie |
