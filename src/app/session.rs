...
impl App {
    ...
    pub(super) fn apply_session_restore(&mut self, file_arg_given: bool) {
        ...
        // preserves it), so a restart round-trips the drawn stack exactly like
        // every other consumer of the one order (`session_buffers`' own doc).
        // `WorkingSet::open` on an ALREADY-open key only updates its
        // root/path in place without moving it, so enrolling every survivor
        // here — including whichever one is (or becomes) active — is safe to
        // do BEFORE `App::new`'s later `enrol_active` call: that call's own
        // `open()` for the true active key lands on this same slot instead of
        // appending a fresh one, so it cannot disturb the order just restored.
        // A CLI-argument file with no matching survivor is not enrolled here
        // at all; `enrol_active` gives it a fresh slot afterward, at the end
        // of the restored order — the same "a file takes a slot when it is
        // FIRST opened" rule an ordinary `load_path` follows.
...
mod tests {
    ...
    /// **THE WORKING SET'S OWN ORDER SURVIVES A RESTART**, including a
    /// USER DRAG performed before the flush — the same order every OTHER
    /// consumer reads (resting window, expanded view, grouped view) is what
    /// session restore round-trips too, not the registry's own separate MRU.
    /// Before `session_buffers`/`apply_session_restore` were fixed to read/
    /// write through `WorkingSet` rather than `registry.iter()`, this drag's
    /// own reorder was invisible to the save entirely (the registry's MRU
    /// order does not move on a drag) and every BACKGROUND survivor lost its
    /// working-set row on restore outright (only the active file was ever
    /// re-enrolled) — so a relaunch silently collapsed a multi-file stack
