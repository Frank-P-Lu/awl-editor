...
mod bundled_doc_law {
    ...
    fn this_file_source() -> String {
        ...
        let full = fs::read_to_string(root.join("src/app/files/open.rs")).expect("read open.rs");
        let marker = "mod bundled_doc_law";
        let end = full
            .find(marker)
            .expect("this law's own module marker must be findable in its own file");
        full[..end].to_string()
    }
    ...
    fn functions(source: &str) -> Vec<(String, String)> {
        ...
        for line in source.split_inclusive('\n') {
            ...
            let is_sig = trimmed.starts_with("fn ")
    ...
    fn only_open_bundled_doc_writes_an_embedded_document_and_loads_it() {
        ...
        let mut offenders: Vec<String> = fns
            ...
            .filter(|(_, body)| {
                body.contains("fs::write_atomic") && body.contains("self.load_path(")
            })
