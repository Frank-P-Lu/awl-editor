...
pub(super) fn write_sidecar(
    ...
    // Effective document metrics share the PNG coordinate space; base constants
    // would address the wrong row at non-default zoom.
    // Read them only after the pipeline is shaped.
    let (font_zoom, font_size, line_height) = pipeline.effective_font_metrics();

    // The schema template lives in the small scroll-sidecar seam.
    // Named arguments remain here beside their owning reports.
...
/// FOLDS block: the FULL-document logical lines of the collapsed (folded) ATX
/// headings, ascending — a JSON array (`[]` when nothing is folded, so a default
/// capture stays byte-stable). The `text`/`cursor`/`selection` above are already
/// the fold-FILTERED view (hidden lines dropped → zero height); this lets a
/// reviewer assert which sections are collapsed straight from the sidecar.
