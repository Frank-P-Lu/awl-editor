...
//!
//! Dismissal is intentionally NOT scoped to Esc: `actions::apply_transition` closes
...
//! `about_open()` is checked at the very TOP of `apply_transition`, UNCONDITIONALLY,
...
//! unrelated `apply_transition` call could otherwise walk into the top-of-function
...
//! ask can't close that gap, so `apply_transition` acquires the ONE process-wide guard
