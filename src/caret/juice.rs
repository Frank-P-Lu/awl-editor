...
//! spring: the cosmetic squash-pop (`kick_pop`/`kick_squash`/`step_pop`/
//! `pop_scale`), the typing impact, the deletion squash, the kill-line gulp, the
//! Enter line-landing squash, and the velocity-kick primitives (`kick`/`recoil`)
//! the I-beam recoil and blocked-action bump ride.
//!
//! FOUR PROPERTIES HOLD FOR EVERY FLINCH HERE, so each is stated once rather
//! than on each method:
//!
//! * They ride the VISUAL caret only. The logical cursor and the spring's
//!   `target` are untouched, so none of them adds input latency.
//! * They always decay back to the SAME resting caret, which is what keeps a
//!   settled headless capture free of them.
//! * They fire in every caret look.
//! * The edit flinches are VELOCITY-DAMPED via [`CaretAnim::impact_damp`] so a
//!   fast repeat smooths into a slide instead of strobing. [`CaretAnim::copy_pulse`]
//!   is the deliberate exception and says why.
//!
//! These stay inherent methods on [`CaretAnim`] so a child module still sees its
//! ancestor's private fields. The tunable `CARET_*` magnitudes live in the
//! `caret` root and resolve here via `use super::*`.
...
impl CaretAnim {
    ...
    /// (ms) — the generalized pop the nav bounce, the deletion squash, the
    /// kill-line gulp and the typing impact all share. Resets the progress to 0
    /// (fully squashed to `floor`); [`step_pop`] eases it back to 1.0 over `ms`.
    /// PURELY a draw-time scale: it touches no position or velocity state.
    ...
    /// TYPING IMPACT: the caret FLINCHES as a character is typed — a quick
    /// squash-pop ([`CARET_TYPE_IMPACT_SCALE`]) PLUS a velocity BACK-KICK
    /// ([`CARET_TYPE_IMPACT_KICK`]) AGAINST the forward insertion, so it recoils at
    /// the keystroke and the spring, its target already at the new cell, settles it
    /// forward.
    ...
    /// DELETION SQUASH: a small INWARD squash ([`CARET_DELETE_SQUASH`]) as a
    /// backspace / C-d swallows the character into the caret — the mark compresses
    /// toward the deletion point ("it eats what it deletes"). The OPPOSITE of
    /// typing's outward flinch: a PURE scale collapse with NO velocity kick.
    ...
    /// KILL-LINE GULP: a BIGGER, longer caret pulse ([`CARET_GULP_SCALE`] over
    /// [`CARET_GULP_MS`]) — a single satisfying swallow as a whole line vanishes
    /// into the caret.
    ...
    /// ENTER JUICE — LINE LANDING: a caret-level "touchdown" squash
    /// ([`CARET_LINE_LAND_SCALE`]) as the caret takes the new line under Enter,
    /// springing back to 1.0 over [`CARET_LINE_LAND_MS`].
    ///
    /// ⚠️ PURE scale, NO velocity kick on this axis. Newline's vertical reflow
    /// SNAPS via [`CaretAnim::jump_to`], and a kick here re-introduces exactly the
    /// caret-lags-on-Enter lag that snap exists to remove.
    ...
    /// document caret. At rest the factor is 1.0 — the identity.
    ...
    /// [`CARET_COPY_PULSE_MS`]. PURELY a scale — nothing moved, so no velocity
    /// kick. Paired with a SEPARATE selection-quad tint pulse on the render
    /// pipeline ([`crate::render::TextPipeline::copy_pulse`]).
    ///
    /// THE ONE FLINCH THAT IS NOT VELOCITY-DAMPED: copy is a one-shot deliberate
    /// action rather than a fast-repeat one, so a plain kick reads calmer here
    /// than a damped one would.
    ...
    /// (DeleteBackward). ⚠️ Newline must NOT kick: its vertical reflow SNAPS via
    /// [`jump_to`], and a downward gravity-drop reintroduces the very insertion
    /// -point lag that snap removes. Marks the spring animating so the step loop
    /// runs the kick out. Purely additive to the current velocity, so a kick
    /// mid-glide rides on top of the in-flight motion.
    ...
    /// existing spring settles it back. Purely a velocity kick (reuses [`kick`]),
    /// and ADDITIVE, so a recoil mid-glide rides on top of the in-flight motion.
