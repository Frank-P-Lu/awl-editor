//! Swift — the shared definition walk ([`crate::syntax::scanner`]) under Swift's
//! constants, emitting only the four Alabaster roles and leaving everything else
//! (keywords, operators, identifiers, punctuation) on the default ink:
//!
//! - `Comment`    — `// line` and `/* block */` (Swift nests them, and doc
//!   comments `///` / `/** */` ride here too).
//! - `Str`        — `"strings"`, multiline `"""..."""`, and raw strings with any
//!   number of `#` delimiters (`#"..."#`, `##"..."##`, raw multiline
//!   `#"""..."""#`). An interpolated string (`"\(x)"`) is one `Str` span.
//! - `Constant`   — numeric literals (`0x`/`0o`/`0b`, floats, exponents, `_`
//!   separators, hex floats) and `true` / `false` / `nil`.
//! - `Definition` — the identifier right after a `func` / `class` / `struct` /
//!   `enum` / `protocol` / `extension` / `actor` / `typealias` /
...
pub(super) const SPEC: LangSpec = LangSpec {
    ...
    // Swift's `let` / `var` bindings are included (best-effort: the name after
    // `let x = …`).
    def_kws: &[
        "func",
        "class",
...
fn scan_number(b: &[u8], i: usize) -> usize {
    ...
    while j < n {
        ...
        if c.is_ascii_alphanumeric() || c == b'_' {
            ...
            if j + 1 < n && (b[j + 1] == b'.' || super::is_ident_start(b[j + 1])) {
                break;
            }
            j += 1;
