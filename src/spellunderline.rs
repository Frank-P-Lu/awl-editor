...
impl SpellUnderlinePipeline {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat, srgba: [u8; 4]) -> Self {
        let shader = crate::gpu_cache::shader(device, crate::gpu_cache::Shader::SpellUnderline);

        let bind_group_layout = crate::gpu_cache::bind_group_layout("spellunderline", || {
        ...
        let pipeline = crate::gpu_cache::render_pipeline("spellunderline", format, || {
...
mod tests {
    ...
    fn spelling_wave_tapers_geometry_and_reserves_opacity_fade_for_straight_nits() {
        ...
        let fade = shader.find("a = a * smoothstep(left - 0.5").expect("straight nit fade remains");
        let straight = shader.find("if (in.amp == 0.0)").expect("straight nit branch remains");
        assert!(fade > straight, "opacity fading is enrolled only for the byte-identical zero-amplitude nit path");
    }
