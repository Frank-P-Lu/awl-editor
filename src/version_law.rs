...
//! dependency crate versions in `Cargo.lock`,
//! `scripts/test-native-gate.sh`'s mocked cargo output line (testing string
//! matching, not real version equality), and the literal placeholder
//! strings `updates.rs`/`mac_about/facts.rs` use to exercise URL-encoding
//! and layout logic. Sweeping all of those into "no 0.1.0 anywhere" would
//! make the law fail on itself. Instead this pins the ACTUAL
//! version-bearing surfaces to the real, compiled version — which is a
//! stronger check than a placeholder-string ban: it fails on ANY drift, not
//! just a reversion to exactly "0.1.0".
#![cfg(test)]
