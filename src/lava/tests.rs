...
/// **BIT-IDENTITY, OVER EVERY BYTE.** `srgb_u8_to_linear` used to carry its own
/// inline per-channel loop; it now calls `theme::srgb_channel_to_linear_f32`.
...
#[test]
fn srgb_u8_to_linear_is_bit_identical_to_the_pre_refactor_formula_over_every_byte() {
    ...
    for v in 0u8..=255 {
        ...
        let c = srgb_u8_to_linear(Srgb::rgb(v, v, v));
        ...
        assert_eq!(c[3], 1.0, "opaque: alpha is always 1.0");
    }
}
