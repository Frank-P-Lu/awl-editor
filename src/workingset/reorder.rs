...
//! margin stack row. Split out of `workingset.rs` (item 444 residual 3's own
//! module, already at its natural size before this landed) because a reorder
//! is a cohesive unit — a row→group-slot resolver shared with the drag
//! target, the in-group clamp, and the splice itself — rather than three
//! more methods scattered through the core model file.
//!
...
use super::{OpenFile, Panel, PanelRow, WorkingSet};
