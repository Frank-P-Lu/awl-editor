...
// Its READER is test-only alongside `test_gpu`, its one consumer; the wgpu
// `counters` feature it reads is on in EVERY build (Cargo.toml), so the binary
// under test and the binary that ships are one wgpu configuration.
