...
impl TextPipeline {
    ...
    /// ITEM 221 — CASSOWARY'S ROTATED SECONDARY-LOCATION CUE, the only real
    /// (non-test) caller of item 235's rotated-label capability today. Knows
    /// nothing about facets, plans, or worlds: `text` is whatever
    /// `crate::render::chrome`'s `prepare_overlay_rotated_location` decided
    /// to say, `card_x` is the card's own left edge, `(row_top, row_height)`
    /// is the row band item 220's planner already reserved for this line (no
    /// second row is planned — see `theme::LocationStyle`'s doc), and
    ...
    pub(in crate::render) fn prepare_rotated_location_label(
        ...
        let natural_size = m.font_size * ui * crate::render::chrome::LOCATION_SCALE;
        ...
        // in that taller box (`overlay_shape_theme`'s `mark_cy`), so roughly
        // as much of the gap sits ABOVE the pill (unusable — that's the
        // query field's own breathing room) as below it (the part this run
        // may safely enter). `ROTATED_LOCATION_HEADER_GAP_FRAC` is that
        // share, measured against the real shaped pill rather than derived
        // from its placement formula (which would duplicate
        // `overlay_shape_theme`'s own private geometry here) — calibrated
        // conservative, confirmed against real captures at the roster's two
        // longest facet names ("Navigate", "Settings").
...
///
/// BOTTOM, never centred: item 220's row pitch is one line tall, but a
/// facet name reading bottom-to-top is routinely several lines' worth of
/// glyph advance ("Settings", "Navigate" — longer than "Files", the string
/// this cue was first measured against). Centring let a long name's TOP
/// half grow upward into the card's own open header gap (harmless) while
/// its BOTTOM half grew downward into the very next command row's own
/// plate — the exact crowding this item exists to forbid. Anchoring the
/// run's bottom edge to the row's own bottom means every overflow grows
/// upward, into the calm space above the row (the lens strip's own
/// slack), and never downward into a row that isn't this one.
