...
impl TextPipeline {
    ...
    pub fn sync_theme_colors(&mut self) {
        ...
        self.table_rule_pipeline
            .set_color(theme::muted().rgba_bytes());
        let mut table_empty = theme::muted().rgba_bytes();
        table_empty[3] = 30;
        self.table_empty_pipeline.set_color(table_empty);
    ...
    pub fn set_view(&mut self, view: &ViewState) {
        ...
        // THE WORKING SET'S RAIL CLAIM. Read fresh from the view every sync
        // rather than latched: a buffer opening, closing or being activated
        // changes it without touching this document's text, so there is no edit
        // to hang an invalidation on — and nothing to go stale.
