...
fn only_diagonal_spell_popups_use_measured_clusters_and_local_frost() {
    ...
    let Some((device, queue, mut p)) = headless_dqp(LOGICAL_W as f32, LOGICAL_H as f32) else {
        eprintln!(
            "skipping only_diagonal_spell_popups_use_measured_clusters_and_local_frost: no wgpu adapter"
        );
        return;
    };
                ...
                match world.render_caps.list_style {
                    crate::theme::ListStyle::Diagonal(spine) => {
                        assert!(
                            ...
                            "{label}: widest action was clipped/elided; shaped {rows:?}; measured {measured}, cluster budget {:?}, card {card:?}",
                            p.diagonal_cluster_budget(&p.overlay_geometry(w), rows.len())
                        );
                        assert!(
                            rows.iter().all(|row| !row.contains('…')),
                            "{label}: an ordinary short/long roster must not elide; shaped {rows:?}"
                        );
                        assert!(
                            card[2] + 0.5 < legacy,
                            "{label}: measured Diagonal popup did not tighten the retired broad span: card {} legacy {legacy} measured {measured} grid {grid}",
                            card[2]
                        );
                        let Some(crate::render::blur::Frost::Footprint(foot)) = p.frost_mode()
                        else {
                            panic!("{label}: Diagonal spell popup did not route to local frost")
                        };
                        assert!(
                            !p.dims_doc(),
                            "{label}: local spell frost must never report full-document dimming"
                        );
                        assert!(
                        ...
                        assert_eq!(
                            ...
                            "{label}: frost must rake in the row spine's authored direction"
                        );
                        enrolled.push(label);
                    }
                    ...
                    | crate::theme::ListStyle::Rules(_) => {
                        assert_eq!(
                            card[2].to_bits(),
                            legacy.to_bits(),
                            "{label}: non-Diagonal spell geometry left the historical expression"
                        );
                        assert_eq!(
                            p.frost_mode(),
                            None,
                            "{label}: non-Diagonal spell popup enrolled in the new frost"
                        );
                        inert.push(label);
                    }
                }
    ...
    assert_eq!(
        enrolled.len(),
        8,
        "two Diagonal worlds × dpi1/2 × short/long"
    );
    assert_eq!(
        inert.len(),
...
fn diagonal_spell_frost_changes_its_rake_and_nothing_beyond_the_feather() {
    ...
    let Some((device, queue, mut p)) = headless_dqp(LOGICAL_W as f32, LOGICAL_H as f32) else {
        eprintln!(
            "skipping diagonal_spell_frost_changes_its_rake_and_nothing_beyond_the_feather: no wgpu adapter"
        );
        return;
    };
    crate::menubar::set_menu_bar_on(false);
    ...
    for world in crate::theme::THEMES.iter().filter(|world| {
        ...
        for dpi in [1.0_f32, 2.0] {
            ...
            for (word, suggestions) in [
                ...
                let label = format!("{} @ {dpi}x, {word}", world.name);
                eprintln!(
                    "MEASURED {label}: frost-changed {inside_changed} pixels inside mask, {outside_changed} outside"
                );
                assert!(
                    inside_changed > (100.0 * dpi * dpi) as usize,
                    "{label}: local frost has no visible subject; only {inside_changed} changed pixels"
                );
                assert_eq!(
                    outside_changed, 0,
