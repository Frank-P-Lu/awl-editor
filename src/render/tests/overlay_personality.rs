...
// --- A SUMMONED WORKSPACE NEVER DRAWS A PLACARD ----------------------------
//
// A workspace's own box already fills the canvas down to a thin margin, so
// the giant corner wordmark's authored canvas-corner bleed has nowhere left
// to go but past the workspace's own edge — a canvas-anchored "SETTINGS"
// spilling off the bottom-left corner on every Placard world.
//
// `placard_style_applies` (`overlay_shape.rs`) is the one predicate both
// `overlay_shape_placard` (draws it) and `overlay_title_prefix` (the modest
// inline title a workspace shows instead) reduce to, so the two cannot show
// at once and cannot drift from each other's idea of when a placard fires.

/// Sweeps every SHIPPED Placard world (derived from `theme::THEMES`, never a
/// named one) through the SAME real end-to-end shaper the query line draws
/// with (`query_line`, this file's own `overlay_shape_text` witness): on an
...
#[test]
fn a_summoned_workspace_never_draws_a_placard_on_any_shipped_placard_world() {
    let _g = crate::testlock::serial();
    let Some(mut p) = headless_pipeline() else {
        eprintln!(
            "skipping a_summoned_workspace_never_draws_a_placard_on_any_shipped_placard_world: \
             no wgpu adapter"
        );
        return;
    };
    ...
    let placard_worlds: Vec<&str> = theme::THEMES
        .iter()
        .filter_map(|t| match t.render_caps.title_style {
            theme::TitleStyle::Placard { .. } => Some(t.name),
            theme::TitleStyle::InlinePrefix => None,
        })
        .collect();
    assert!(
    ...
    for world in &placard_worlds {
        ...
        // PRECONDITION: an ordinary contextual card on this world still draws
        // its wordmark — otherwise excluding a workspace proves nothing.
        let mut contextual = view("hello\n", 0, 0);
        contextual.overlay_active = true;
        contextual.overlay_title = "settings".to_string();
        contextual.overlay_items = (0..10).map(|i| format!("Command {i}")).collect();
        p.set_view(&contextual);
        let cgeom = p.overlay_geometry(1200);
        assert!(
            p.overlay_shape_placard(&cgeom).is_some(),
            "{world}: precondition — an ordinary contextual card must still draw its placard"
        );

        // THE SAME TITLE, presented as a workspace: no placard, ever — and
        // the query line falls back to the modest inline title instead of
        // announcing nothing at all.
        let ov = super::workspace::workspace_card(0, true);
        let mut wv = super::workspace::workspace_view(&ov);
        wv.overlay_title = "settings".to_string();
        p.set_view(&wv);
        let wgeom = p.overlay_geometry(1200);
        assert!(
            wgeom.workspace,
            "{world}: precondition — the fixture must actually be a workspace"
        );
        assert!(
    ...
    assert_eq!(
        checked,
        placard_worlds.len(),
        "every shipped placard world named above must be graded"
    );
    theme::set_active(theme::DEFAULT_THEME);
    set_title_style_test_override(None);
}
...
fn placard_wordmark_stays_in_bounds_at_the_apps_own_minimum_window_size() {
    ...
    let min_h = MIN_LINES * LINE_HEIGHT + 2.0 * TEXT_TOP.0;
