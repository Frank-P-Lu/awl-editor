...
fn every_workspace_footer_is_reserved_before_rows_and_drawn_inside_its_card() {
    ...
    let mut rows = std::collections::BTreeMap::<(Surface, usize, u32), RowPair>::new();
    let mut graded = 0usize;
    let mut pixel_cells = 0usize;
    let mut rules_reduced = 0usize;
    let mut bars_reduced = 0usize;
    let mut history_reduced = 0usize;
        ...
        for surface in [Surface::Settings, Surface::History] {
            for cell in CELLS {
                for dpi in [1.0f32, 2.0] {
                    ...
                    let (width, height) =
                        ((cell.w as f32 * dpi) as u32, (cell.h as f32 * dpi) as u32);
                    ...
                    if surface == Surface::History && cell.geometry == Geometry::Minimum {
                        assert!(
                            ...
                            "{ctx}: the minimum History control must exercise its narrow stage"
                        );
                    }

                    let Some((content_bottom, hint_top, hint_bottom)) =
                        pipeline.overlay_hint_gap_probe(width)
                    else {
                    ...
                    assert!(
                        ...
                        "{ctx}: narrow composition changed footer cell order: authored {:?}, drawn {drawn_hint:?}",
                        v.overlay_hint
                    );
                    ...
                    assert!(
                        ...
                        "{ctx}: width yield omitted the consequential final `{required}` cell from {drawn_hint:?}"
                    );
                    assert!(
                        hint_bottom <= card[1] + card[3] + 0.5,
                        "{ctx}: footer line {hint_top:.1}..{hint_bottom:.1} runs beyond card bottom {:.1}",
                        card[1] + card[3]
                    );
                    assert!(
                        hint_top >= content_bottom,
                        "{ctx}: footer begins at {hint_top:.1} before candidate content ends at {content_bottom:.1}"
                    );
                    ...
                    for (display, row) in plan.rows().iter().enumerate() {
                        ...
                        assert!(
                            row.top >= card[1] && row.bottom() <= hint_top + 0.5,
                            "{ctx}: row {display} at {:.1}..{:.1} leaves its row band before the footer at {hint_top:.1}",
                            row.top,
                            row.bottom()
                        );
                        if let Some(next) = plan.rows().get(display + 1) {
                            assert!(
                                ...
                                "{ctx}: rows {display}/{} advance {:.2}px instead of the world's {pitch:.2}px rhythm",
                                display + 1,
                                next.top - row.top
                            );
                        }
                    ...
                    assert!(
                        inked >= 8,
                        "{ctx}: only {inked} footer columns carry a glyph edge; shaped state without visible teaching ink is not presence"
                    );
                    pixel_cells += 1;
    ...
    for ((surface, world_index, _dpi), pair) in &rows {
        assert!(
            pair.roomy > 0,
            "{} {surface:?}: roomy control plans no candidates",
            crate::theme::THEMES[*world_index].name
        );
        if pair.minimum < pair.roomy {
            match (
                surface,
                crate::theme::THEMES[*world_index].render_caps.list_style,
            ) {
                (Surface::History, _) => history_reduced += 1,
                (_, theme::ListStyle::Rules(_)) => rules_reduced += 1,
                (_, theme::ListStyle::Bars) => bars_reduced += 1,
                _ => {}
            }
        }
    }
    assert!(
        rules_reduced >= 2
            && bars_reduced >= 6
            && history_reduced >= crate::theme::THEMES.len() * 2,
        "the minimum must prove candidate yielding in Rules, all three Bars worlds, and narrow History at both DPIs; got rules={rules_reduced}, bars={bars_reduced}, history={history_reduced}"
    );
    assert_eq!(
        graded,
    ...
    assert_eq!(pixel_cells, graded, "every graded footer must reach pixels");

    drop(world_pin);
    crate::menubar::set_menu_bar_on(ambient_menu_bar);
    eprintln!(
        "workspace footer reserve: {graded} outcomes and {pixel_cells} pixel cells; reductions rules={rules_reduced}, bars={bars_reduced}, history={history_reduced}; ambient menu bar {ambient_menu_bar}"
    );
}
