...
fn empty_table_wash_has_rendered_presence_on_every_world() {
    ...
    for world in theme::THEMES {
        ...
        let wash =
            pixels[top as usize * w as usize + (left + report[0].col_widths[0] * 0.5) as usize];
        let ground = pixels[top as usize * w as usize
            + (left + report[0].col_widths.iter().sum::<f32>() + 12.0) as usize];
