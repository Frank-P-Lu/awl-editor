...
fn split_bounds_carve_the_query_beat_above_the_first_row() {
    ...
    for header_rows in [1usize, 2] {
        ...
        // It is `SPLIT_GAP_FRAC` (0.35, tuned to buy the facet strip real
        // clearance from the lower surface's own rim) of the beat tall — a
        // strip of ground, not the whole beat.
        assert!(
...
fn faceted_query_strip_is_optically_centered_not_bottom_heavy() {
    ...
    // width (still exactly `0.35 * header_gap`) — only its position shifted.
    assert!(
