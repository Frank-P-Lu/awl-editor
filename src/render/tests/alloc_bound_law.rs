...
//!    it back, and N tests hold N times one test. The portable sum catches the
//!    buffer-heavy shape; a tighter view-class clause catches a view-only pin
//!    that could fit inside the sum's asynchronous slack.
//!
...
fn assert_views_do_not_accumulate(samples: &[gpu_alloc::GpuLive], subject: &str) {
    let first = samples
        .first()
...
fn the_allocation_counter_responds_on_this_backend() {
    ...
    eprintln!("awl gpu-alloc probe: {probe}");
...
fn the_render_suite_does_not_accumulate_gpu_objects_across_tests() {
    let _g = crate::testlock::serial();
    let Some((device, _queue, probe)) = instrumented() else {
        eprintln!(
    ...
    let mut live_samples: Vec<gpu_alloc::GpuLive> = Vec::with_capacity(worlds.len());
    ...
    for t in worlds.iter() {
        ...
        live_samples.push(gpu_alloc::live(&device));
    }
    let samples: Vec<i64> = live_samples
        .iter()
        .map(|sample| sample.portable())
        .collect();
    let first = samples[0];
    ...
    if probe.responds(Class::TextureViews) {
        assert_views_do_not_accumulate(&live_samples, "the render-suite sweep");
    }
}
