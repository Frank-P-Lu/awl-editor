    ...
    for world in &worlds {
        ...
        for (cw, ch) in [(1200u32, 800u32), (900u32, 460u32)] {
            ...
            for row in plan.rows().iter().filter(|r| r.item.is_some()) {
                ...
                let residue = box_residue(&a, &b, cw, ch, row_box);
                assert!(
                    residue == 0,
                    "{world} {cw}x{ch}: candidate row {} changed by {residue}px when the \
                     picker's own placard drew — its label is not fully masked from the \
                     busiest ground in the room",
                    row.display
                );
                rows_checked += 1;
            }
