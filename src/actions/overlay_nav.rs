...
pub(super) fn overlay_intercept(ctx: &mut ActionCtx, action: &Action) -> Effect {
    ...
    if let Some(effect) = workspace_intercept(ctx, action) {
        return effect;
    }
...
fn workspace_intercept(ctx: &mut ActionCtx, action: &Action) -> Option<Effect> {
    let ov = ctx.journey.card().unwrap();
...
/// ITEM 114 — OPEN THE SETTINGS WORKSPACE STANDING ON `row`: its category
...
fn deep_link_settings(ctx: &mut ActionCtx, row: crate::settings::SettingRow) -> bool {
...
fn dispatch_settings_row(ctx: &mut ActionCtx, row: crate::settings::SettingRow) -> Effect {
    match row.kind {
        ...
        crate::settings::SettingKind::Value | crate::settings::SettingKind::Range => {
            // ITEM 114 — THE Cmd-P DEEP LINK. A Range row's control is item 94's
            // rail, and the palette structurally cannot show one: its settings
            // rows are appended by `attach_settings_rows`, which carries no
            // `RangeCell`, so `item_range_fracs` reports nothing and the row
            // offers a bare text field with no thumb, no band and no neighbours.
            // Rather than teach the palette a second copy of a Settings control,
            // the row now takes you to the place that owns it — standing on its
            // own category and its own row. Every other kind keeps its existing
            // palette path: a Toggle completes in one keystroke, a Picker opens
            // the real audition, an Action runs, a Path opens the real navigator.
            if deep_link_settings(ctx, row) {
                return Effect::None;
            }
