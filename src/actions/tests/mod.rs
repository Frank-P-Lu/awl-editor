...
/// a new motion.
macro_rules! assert_action_roster {
    ($action:expr) => {
        match $action {
            Action::ForwardChar
            | Action::KeepTutorial
...
fn assert_action_roster_covers(action: &Action) {
    assert_action_roster!(action);
}
...
pub(super) fn smoke_command_kind(action: &Action) -> SmokeKind {
    classify_smoke_command!(action)
}
