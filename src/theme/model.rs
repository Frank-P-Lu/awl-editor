...
enum_with_all! {
    /// The icon tile's GROUND, as a theme-owned capability (item 121): every
    /// world's shipped default is `Base100` (the tile ground is the world's
    /// darkest plane, same as any other summoned surface). A world may opt
    /// into one of the two bounded blends toward its own `base_300` instead —
    /// never a hand-picked hex, never a Firetail-name branch in the exporter,
    /// only this closed preset roster read back through
    /// [`Theme::icon_ground_color`].
    #[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
    pub enum IconGround {
        Base100,
        Blend25,
...
impl Theme {
    ...
    /// into a blend toward `base_300` via [`IconGround`] (item 121). The ONE
    /// place that math happens — the exporter's manifest, the packer's pixel
    /// laws, and any live preview all read this rather than restating the
    /// blend fraction.
