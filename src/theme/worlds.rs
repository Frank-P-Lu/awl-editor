...
pub const GUMTREE: Theme = Theme {
    ...
    // EUCALYPTUS ZIGZAG (item 86; FIELD-tiled by item 89, dials re-derived by
    // its coverage repair) — the grass-BANDS field (item 69) RETIRES here: a
    // distinct, BROADER and QUIETER chevron ground replaces it, so the two
    // light zigzag worlds (this one and Quokka's) read as separately authored
    // fields, not a recolor of one asset. A BROAD, OPEN profile — a 170px
    // tooth wavelength against 60px of excursion, i.e. the SAME ~0.35
    // profile ratio item 86 authored (85/250), the whole field simply scaled
    // to a pitch a page margin can hold. The excursion is what sets the ROW
    // PITCH (the shader's abutment rule, `row_h = 2*amp + thickness`: the
    // pitch is DERIVED, never authored, so no dial pair can reopen the
    // blank-lane defect), giving ~126px rows against Quokka's ~50px — this
    // world's field is 2.5x the scale of Quokka's across travel, and 1.7x
    // along it. A SHALLOW ~15° near-horizontal travel angle (vs. Quokka's
    // steep ~54° diagonal — branch-like, not a chevron stitch), a fine ~6px
    // ribbon, and a LOW 0.20 density (a third of Quokka's 0.60 — a quiet
    // whisper mark over the gradient, not an opaque field). ONLY the
    // eucalyptus ground ladder — `base_100`/`base_200`/`base_300` verbatim,
    // no separately-tuned tint, same restraint the old Bands field kept.
...
pub const QUOKKA: Theme = Theme {
    ...
    // the whole margin. Both dials are UNCHANGED through item 89's coverage
    // repair — Quokka's field was already fine-grained enough to cover every
    // margin cell at every swept viewport; only Gumtree's broad one wasn't. A
    // TIGHT, playful profile — a 100px tooth wavelength (Gumtree's is 170px)
    // with 24px of excursion, a sharper ~0.24 profile ratio than Gumtree's
    // ~0.35 open "V". That excursion also sets the ROW PITCH (the shader's
    // abutment rule, `row_h = 2*amp + thickness`), giving a close-set ~50px
    // row against Gumtree's ~126px — the TIGHT pole in both directions at
    // once, ~9 close-set chevron rows down an ordinary window against
    // Gumtree's ~6. A fine ~2.4px ribbon, a steep ~54° travel angle (echoes
    // the old dot grid's own (0.7,0.7) diagonal), and the round's boldest
    // density (0.60) — deliberately the loud pole of the two zigzag worlds so
    // it never reads as a recolor of Gumtree's broader, quieter, shallower
    // field below (distinct scale/profile/direction/contrast, all four dials).
    background: Background::Zigzag {
