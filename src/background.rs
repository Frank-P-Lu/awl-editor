...
struct Globals {
    ...
    /// `params.y` = stripe/band angle in radians (Stripes, Bands), `params.z`
    /// = WAVES phase-drift radians (item 87; `0.0` for every non-Waves ground
    /// AND every Waves frame the ambient clock hasn't advanced — see
    /// [`BackgroundPipeline::prepare`]), `.w` reserved. Bands/Waves read
    /// `from`/`to`/`tint` above as their three authored TONES (not a
    /// gradient) — no new uniform slots needed.
    params: [f32; 4],
}
...
// uploaded through `params.z` (see `ground_params`'s doc). The two boundary
...
// read off `params.z` and its `waves_rgb`'s
...
/// Pack the STATIC per-ground params the shader reads: `x` = the Dots
/// proximity flag (0/1), `y` = the Stripes angle in radians, `z` = 0.0 (a
/// per-THEME placeholder — [`BackgroundPipeline::prepare`] overwrites it with
/// the per-FRAME Waves drift every upload), `w` reserved. For every
/// UNCHANGED ground `x`/`y` are both 0, so the shader takes its exact
/// original code path (a byte-identical render).
